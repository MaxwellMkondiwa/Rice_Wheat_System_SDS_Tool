...
# Impact on variance
Biv_Rice_sow_date_pred$sigma1_CI <- predict(biv_gauss_yld_non_struct, newdata = Biv_Rice_sow_date_pred, model = "sigma1", term = "sowdate_fmt_rice_day",
   FUN = c95, intercept = FALSE)

 ylim <- range(c(Biv_Rice_sow_date_pred$sigma1_CI))
 
png(file="figures/Biv_Rice_sow_date_pred_sigma1_plot_unstruct.png", width=350, height=250)
 
bamlss::plot2d(sigma1_CI ~ sowdate_fmt_rice_day, data = Biv_Rice_sow_date_pred, ylim = ylim,xlab="Rice transplanting date", lwd = c(1,3,1),col.lines = "#2E9FDF", main="Effect on rice yield variance")
...
library(janitor)
Biv_Rice_sow_date_pred_clean=clean_names(Biv_Rice_sow_date_pred)

Biv_Rice_sow_date_pred_clean=as.data.frame(Biv_Rice_sow_date_pred_clean)
# ggplot2
Biv_Rice_sow_date_pred_clean$rho_ci_mean=Biv_Rice_sow_date_pred_clean$rho_ci$Mean

Biv_Rice_sow_date_pred_clean$rho_ci_2_5=Biv_Rice_sow_date_pred_clean$rho_ci$`2.5%`

Biv_Rice_sow_date_pred_clean$rho_ci_97_5=Biv_Rice_sow_date_pred_clean$rho_ci$`97.5%`

library(ggplot2)
ggplot(Biv_Rice_sow_date_pred_clean, aes(x=sowdate_fmt_rice_day))+
  geom_line(aes(y=rho_ci_mean),color="black", lwd=1.2)+
  geom_line(aes(y=rho_ci_2_5),color="grey",linetype = "dotted", lwd=1.2)+
  geom_line(aes(y=rho_ci_97_5),color="grey",linetype = "dotted", lwd=1.2)+
  theme_bw(base_size = 16)+
  geom_rug()+
  labs(x="Rice transplanting date", y="Effect on rice-wheat \n correlation")

ggsave("figures/transplantingdate_effect_on_correlation_plot.png", dpi=300, height=4,width=5)
...
library(tmap)
tmap_mode("plot")
rice_wheat_yield_rho_dist_sf_sp_map <- tm_shape(rice_wheat_yield_rho_dist_sf) +
  tm_polygons(fill = "Mean_Rice_Wheat_Rho",fill.scale=tm_scale_intervals(values="viridis", style="quantile"), fill_alpha = 0.7,fill.legend = tm_legend (title = "Rice-wheat yield correlation",position=tm_pos_in("right","top"), bg.color ="NA",frame=FALSE)) +
tm_borders("grey")

rice_wheat_yield_rho_dist_sf_sp_map

tmap_save(rice_wheat_yield_rho_dist_sf_sp_map, "figures/Bivariate_rice_wheat_yield_rho_dist_sf_sp_map.png", width=6, height=5)
...
#sigma1 unstr
p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_sigma1_unstr <- tm_shape(p_unstr_str_biv_gauss_yld_non_struct_dt_sf) +
  tm_polygons(fill = "sigma1.Mean",fill.scale=tm_scale_intervals(values="viridis",style="quantile"),fill_alpha = 0.7, fill.legend=tm_legend(title = "Unstructured spatial effects \n on rice yield risk",position=tm_pos_in("right","top"), bg.color ="NA", frame=FALSE)) +
  tm_borders("grey")

p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_sigma1_unstr

tmap_save(p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_sigma1_unstr, "figures/p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_sigma1_unstr.png", width=6.50, height=6)


p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_sigma1_unstr_sign <- tm_shape(p_unstr_str_biv_gauss_yld_non_struct_dt_sf) +
tm_polygons(fill = "sigma1_sign",fill.scale=tm_scale_intervals(values="viridis",style="quantile"),fill_alpha = 0.7, fill.legend=tm_legend(title = "Unstructured spatial effects \n on rice yield risk",position=tm_pos_in("right","top"), bg.color ="NA", frame=FALSE)) +
  tm_borders("grey")
...
library(tmap)
tmap_mode("plot")
p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_rhomean <- tm_shape(p_unstr_str_biv_gauss_yld_non_struct_dt_sf) +
 tm_polygons(fill = "rho.Mean",fill.scale=tm_scale_intervals(values="viridis",style="quantile"),fill_alpha = 0.7, fill.legend=tm_legend(title = "Spatial effects",position=tm_pos_in("right","top"), bg.color ="NA", frame=FALSE)) +
  tm_borders("grey")+
  tm_title("Unstructured spatial effects on R&W yield correlation")

p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_rhomean

tmap_save(p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_rhomean, "figures/p_unstr_str_biv_gauss_yld_non_struct_dt_sf_sp_map_rhomean.png", width=5, height=4)
...
library(tmap)
tmap_mode("plot")
spatial_effects_on_rho_map=tm_shape(p_unstr_str_biv_gauss_yld_non_struct_dt_sf) +
  tm_polygons(
    fill = tm_vars(c("rho.Mean _ str","rho.Mean")),fill.scale = tm_scale_intervals(values="viridis", label.style = "continuous",style = "quantile"),fill_alpha=0.7,fill.legend=tm_legend(title=" ", width=20,bg.color="NA",frame=FALSE)) +
  #tm_text("NAME_2",size = 0.5)+
  tm_borders("grey")+
  tm_facets(ncol = 1) +
  tm_layout(panel.labels = c(" (a) Structured spatial effects on rice-wheat yield correlation", "(b) Unstructured spatial effects on rice-wheat yield correlation"),fontsize = 1.2,  legend.outside = TRUE,legend.outside.size = 0.2)
...
spatial_effects_on_rho_map

tmap_save(spatial_effects_on_rho_map,"figures/spatial_effects_on_rho_map.png", dpi=300, height=7, width=7)
```
...
ggplot(Biv_Rice_wheat_sow_date_pred_rev) + 
  ...
  scale_fill_gradientn(colors = hcl.colors(20, "RdYlGn")) +
  coord_fixed()+
  labs(x="Rice transplanting date",y="Wheat sowing date")+
  theme_bw()
...
sowdate_heatmap_corr_rev=ggplot(Biv_Rice_wheat_sow_date_pred_rev,    aes(sowdate_fmt_rice_day,sowdate_fmt_wheat_day, z = rho_CI$Mean )) +
  geom_contour_fill(aes(fill = after_stat(level)),bins=5)+
 # coord_fixed()+
  labs(x="Rice transplanting date",y="Wheat sowing date",title="Planting date effect on revenue correlation",fill="Correlation")+
...
sowdate_heatmap_mu1_mu2_rev=ggplot(Biv_Rice_wheat_sow_date_pred_rev,    aes(sowdate_fmt_rice_day,sowdate_fmt_wheat_day, z = total_revenue_effect )) +
  geom_contour_fill(aes(fill = after_stat(level)),bins=5,binwidth=5)+
 # coord_fixed()+
  labs(x="Rice transplanting date",y="Wheat sowing date",title="Planting date effect on total rice-wheat revenue",fill="Revenue gain/loss \n (000)")+
  theme_bw(base_size = 14)
...
sowdate_heatmap_sigma1_sigma2_rev=ggplot(Biv_Rice_wheat_sow_date_pred_rev,    aes(sowdate_fmt_rice_day,sowdate_fmt_wheat_day, z = total_revenue_std_dev_effect )) +
  geom_contour_fill(aes(fill = after_stat(level)),bins=5)+
  #coord_fixed()+
  labs(x="Rice transplanting date",y="Wheat sowing date",title="Planting date effect on revenue std.deviation",fill="Standard deviation \n (000)")+
  theme_bw(base_size = 14)
...
library(cowplot)
# One plot
cowplot::plot_grid(sowdate_heatmap_mu1_mu2_rev,sowdate_heatmap_sigma1_sigma2_rev,sowdate_heatmap_corr_rev, ncol = 1, labels=c("a","b","c"))


ggsave("figures/sowdate_heatmap_cowplot_final.png", width=6,height=8)
